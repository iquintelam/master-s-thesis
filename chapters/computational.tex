...
\section{Expanded Ensemble Method}\label{ee}
    ...
    \label{eqn:cumfun}
    \end{equation}
    where $K$ is the total number of $\lambda$ states. We obtained these cumulative probabilities for every $\lambda$ set we estimated in order to carry out solvation free energy simulations. A graphical demonstration of the cumulative probability calculated using Eq. \ref{eqn:cumfun} is presented in Figure \ref{fig:optimized_cdfexeample}.
    
    \begin{figure}[h]
    	\centering
    	\includegraphics[width=0.7\linewidth]{Figures/optimized_cdfexeample}
