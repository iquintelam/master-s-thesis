...
\section{SAFT-$\gamma$ Mie Force Field}
	...
	In the equation above, $g_{d,ij}^{HS}$ is obtained from 
	...
	\label{eqn:ghs}
	\end{equation}
	where with $x_{0,ij} = \sigma_{ij}/d_{ij}$ and $k_{1}, k_{2},$ and $k_{3}$ are density dependent coefficients. For brevity, we are not going to detail these coefficients and the other terms of Eq. \ref{eqn:gmie}, but they are available at the original paper \cite{lafitte2013}.  
