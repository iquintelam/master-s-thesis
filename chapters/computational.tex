...
\label{ee}
\end{equation}   
where N is the number of alchemical states and $\eta_{i}$ is the arbitrary weight of the subs ensemble $Z_{i}$ at each state. This weights allow the sampling of the states during the simulation with equal frequency. A sufficient number of states needs to be sampled in order to obtain the reduced free energies differences between the states ($\hat{f}_i-\hat{f}_j$) from the simulations' results.  The weights are calculated doing trials simulations. For the first simulation, the values of $\eta$ are chosen or set to zero and the histogram of the states visited is obtained. With this histogram is possible to estimate the free energy differences and, since the weights are related to the free energies by Eq. \eqref{weight}), the next values of $\eta$ can be calculated  from the previous result. This iteration goes on until a uniform distribution is secured.

equilibrium properties are no longer estimated via arithmetic averages, but via properly weighted averages. 

\begin{equation}
(\eta_{i} - \eta_{j})_{k+1} = \beta(\hat{f}_i-\hat{f}_j)_{k}
\label{weight}
\end{equation}
These acquired  weights  can then  be used in a longer simulation to obtain the data necessary for the free energy differences calculations.
