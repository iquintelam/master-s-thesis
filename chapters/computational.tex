...
With the below equations, it is then possible to calculate the trajectories. During the integration process, macroscopic properties such as temperature and pressure are held constant, and the statistical ensemble are defined according to those properties. In the NVE statistical ensembles, the number of particles (N), volume (V) and energy (E) are held constant.   

Microcanonical ensemble provides a starting point from which all other equilibrium
ensembles are derived.
