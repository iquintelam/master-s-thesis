...
In order to obtain the macroscopic properties from molecular dynamics, we need to have the relation of the microcanonical partition function to the entropy:
...
In the microcanonical ensemble, we can calculate th temperature using thermodynamic averages, specifically through this relation between temperature and kinetic energy per particle:

\begin{equation}
k_{b}T = m \langle{ v ^{2} \rangle}
\end{equation}

However, the isothermal-isobaric and canonical ensembles have external conditional being applied to it (temperature and pressure). For temperature control, the method employed  is to mimic the effect of a thermal reservoir through a thermostat. When using a heat bath to control temperature in a canonical ensemble of a finite system, the kinetic temperature will fluctuate. Hence we need thermostats capable of capturing the correct energy fluctuations in the system. Among the available thermostat are the Berendsen \cite{doi:10.1063/1.448118}, the Andersen \cite{1980JChPh722384A} and the Nose \cite{1984JChPh81511N} thermostats, but here we are going to discuss the most widely used thermostat: the Nosé-Hoover \cite{PhysRevA.31.1695}. This thermostat is based on the formulation of Nosé \cite{1984JChPh81511N}, that used a Lagrangian that contains additional, artificial coordinates and velocities \cite{frenkel}. In this method the Halmitonian in a canonical ensemble is constructed as:
