...
After this results, it need of the interaction parameter was clear. The estimation the SAFT VR Mie EoS using vapor pressure data also didn't provide good results, then it was decided the strategy of estimating the $k_{ij}=0$ with the output form solvation free energy calculations with molecular dynamics as described in the last paragraph of section \ref{solvme}. A $k_{ij}$ individual value for each solvent was initially found, but, since the parameters values for aromatic solutes seems to be very similar (0.148, 0.162, 0.152), the average of this values was taken in order to obtain a general parameter for the pair water+aromatic and avoid the necessity of doing new molecular simulations to find interactions parameters of other aromatic compounds. The $k_{ij}$ estimated are exposed below:
